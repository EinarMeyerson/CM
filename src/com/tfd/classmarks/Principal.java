...
public class Principal extends FragmentActivity implements FragmentProvider {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		principal = (RelativeLayout)findViewById(R.id.princi);
		fondoPrin = principal.getBackground().getCurrent();
	...
	protected void onResume() {
		super.onResume();
		mPager.setCurrentItem(resetPosition);
	...
	public void removeCurrentItem() {
		...
		try{
			...
			Log.d("ANIMATION FRAGMENT EXCEP", "NO EXISTE FRAGMENT A LA IZQUIERDA");
		}
		...
		try{
			...
			Log.d("ANIMATION FRAGMENT EXCEP", "NO EXISTE FRAGMENT A LA DERECHA");
		}
		Log.d("ANIMATION FRAGMENT POSITION", ""+position);
		Log.d("ANIMATION FRAGMENT LASPOSITION", ""+lastPosition);
		...
		if (position == lastPosition && position != 0){
			...
			anim.addListener(new AnimatorListenerAdapter() {
				@Override
				public void onAnimationEnd(Animator animation) {
					Log.d("ANIMACION ULTIMO FRAGMENT", "ANIMATION END");
					frags.remove(position);
					mAdapter.notifyDataSetChanged();
					isEmpty();
				}
...
//			int dialog_width = newAsig.getWidth();
//			int dialog_height = newAsig.getHeight();
//			Rect bounds = new Rect(dialog_height, dialog_width, 0, 0);
//			fondoPrin.setBounds(bounds);
			...
			btn21.setOnClickListener(new View.OnClickListener() {
				...
				public void onClick(View v) {
					...
					if (verdad ==true)
					{
						...
						{
							Log.d("PRUEBA",""+(Float.parseFloat(edtxtporcetaje1.getText().toString())));
							Log.d("porX",""+notamodif2.getPorcentaje());

							Log.d("ERROR",""+porcentajeusado);
							datoscorrectos = false;
						...
						if (datoscorrectos ==true)
						{
							...
							Log.d("updateINT", ""+ControlUpdateNota);
